...
public:
   ...
   eosio_token_tester() {
      ...
      create_accounts( { "alice"_n, "bob"_n, "carol"_n, "proton.token"_n } );
      produce_blocks( 2 );

      set_code( "proton.token"_n, contracts::token_wasm() );
      set_abi( "proton.token"_n, contracts::token_abi().data() );

      produce_blocks();

      const auto& accnt = control->db().get<account_object,by_name>( "proton.token"_n );
      abi_def abi;
   ...
   fc::variant get_stats( const string& symbolname )
   {
      ...
      vector<char> data = get_row_by_account( "proton.token"_n, name(symbol_code), "stat"_n, account_name(symbol_code) );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = eosio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( "proton.token"_n, acc, "accounts"_n, account_name(symbol_code) );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( "proton.token"_n, "create"_n, mvo()
           ( "issuer", issuer)
